...
export function BookingCalendarView() {
    ...
    const findRowForTimeSlot = (startHour: string, endHour: string, timeFormat: 'am' | 'pm') => {
        if (startHour === '8' && endHour === '9' && timeFormat === 'am') return 0;
        else if (startHour === '9' && endHour === '10' && timeFormat === 'am') return 1;
        else if (startHour === '10' && endHour === '11' && timeFormat === 'am') return 2;
        else if (startHour === '11' && endHour === '12' && timeFormat === 'am') return 3;
        else if (startHour === '12' && endHour === '1' && timeFormat === 'pm') return 4;
        else if (startHour === '1' && endHour === '2' && timeFormat === 'pm') return 5;
        else if (startHour === '2' && endHour === '3' && timeFormat === 'pm') return 6;
        else if (startHour === '3' && endHour === '4' && timeFormat === 'pm') return 7;
        else if (startHour === '4' && endHour === '5' && timeFormat === 'pm') return 8;
        else if (startHour === '5' && endHour === '6' && timeFormat === 'pm') return 9;
        else if (startHour === '6' && endHour === '7' && timeFormat === 'pm') return 10;
        else if (startHour === '7' && endHour === '8' && timeFormat === 'pm') return 11;
        else if (startHour === '8' && endHour === '9' && timeFormat === 'pm') return 12;
        else if (startHour === '9' && endHour === '10' && timeFormat === 'pm') return 13;
        else if (startHour === '10' && endHour === '11' && timeFormat === 'pm') return 14;
        else if (startHour === '11' && endHour === '12' && timeFormat === 'pm') return 15;
        else if (startHour === '12' && endHour === '1' && timeFormat === 'am') return 16;
        else if (startHour === '1' && endHour === '2' && timeFormat === 'am') return 17;
        else if (startHour === '2' && endHour === '3' && timeFormat === 'am') return 18;
        else if (startHour === '3' && endHour === '4' && timeFormat === 'am') return 19;
        else if (startHour === '4' && endHour === '5' && timeFormat === 'am') return 20;
        else if (startHour === '5' && endHour === '6' && timeFormat === 'am') return 21;
        else if (startHour === '6' && endHour === '7' && timeFormat === 'am') return 22;
        else if (startHour === '7' && endHour === '8' && timeFormat === 'am') return 23;
    };
    ...
    useEffect(() => {
        barberData?.forEach((barber: any, index: number) => {
            barber?.bookings?.forEach((item: any) => {
                ...
                console.log({ startTime, endTime, timeFormat, startHour, endHour, row });
                setElements((prev: any) => [...prev, { ...item, row: row + 1, col: index }]);
            });
        });
    }, []);
        ...
        <div>
            ...
            <div className="grid grid-cols-12">
                <div className="col-span-1 flex flex-col items-center justify-center border border-b-[1px] border-l-0 border-r-[1px] border-t-0 border-[#DBDBDB] p-4">
                    ...
                    <span className="text-xs font-semibold text-[#666666]">GMT +5:30</span>
                </div>
                <div className="col-span-11">
                    <div className="grid w-full overflow-x-auto"
                        style={{ gridTemplateColumns: `repeat(${barberData?.length || 1}, minmax(300px, 1fr))` }}>
                        {barberData?.map((item) => <CalendarHeaderCard key={item?.id} name={item?.name} time={item?.time} />)}
                    </div>
                </div>
            </div>
            {/* Calendar Header end */}
            ...
            <div className="grid grid-cols-12">
                ...
                <div className="col-span-1 flex h-full flex-col border border-b-0 border-l-0 border-r-[1px] border-t-0 border-[#DBDBDB]">
                    {timeSlots?.map((timeSlot) => (
                        <div key={timeSlot?.id} className="relative flex h-[80px] items-end justify-end gap-x-2">
                ...
                <div className="col-span-11 bg-white">
                    <div className='grid h-0' style={{ gridTemplateColumns: `repeat(${barberData?.length || 1}, minmax(0, 1fr))` }}>
                                    ...
                                    {Array.from({ length: 24 }).map((_, row) => (
                                        <div className="relative cursor-pointer" key={row}>
                                            <div
                                                ...
                                                style={{ top: `${currentTimePosition}px`, transition: 'top 0.5s linear' }}
                                            >
                                                {row === 0 && col === 0 && (
